    ...
    <Transition name="right_data">
      ...
      >
        <div class="flex flex-col gap-2 my-6 lg:mt-32">
          ...
          <span class="px-2">
            <SanityBlocks :blocks="tourData.tourIntro[Trans.currentLocale]" />
          </span>
        </div>
        <ul class="flex flex-col gap-2">
          ...
          <li
            v-for="({ highlights }, idx) in tourData.tourHighlights"
            :key="idx"
        ...
        <div class="flex flex-col gap-2 my-6">
          ...
          <span class="px-2">
            <SanityBlocks
              :blocks="tourData.tourItinerary[Trans.currentLocale]"
            />
          </span>
        </div>
          ...
          >
            ...
            >
              <tr
                ...
                :key="idx"
              >
                <template v-if="tourData.tourPrice[item.price]">
                  <td>{{ item.size }}</td>
                  <td>
                    {{
                      formatCurrency(tourData, tourData.tourPrice[item.price])
                    }}
                    /pax
                  </td>
                </template>
...
<style>
.right_data-enter-active,
.right_data-leave-active {
  transition: all v-bind(`${delayTime/1000}s`) ease;
}

.right_data-enter-from,
.right_data-leave-to {
  opacity: 0;
}
</style>
